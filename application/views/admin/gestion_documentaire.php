					...
					<tr>
						...
						<td>
							<?php if($document['lien']!=''): ?>
							 	...
							 	<?php else: ?>
							 		<a class="btn btn-info" href="<?php echo base_url('/assets/documents/').rawurlencode($document['link']) ?>">Voir</a>
							<?php endif; ?>
						</td>
						<td><?php echo e($categories[$document['category']]) ?></td>
						<td><button class="btn bnt-xs btn-info btnEditDocument" data-id="<?php echo e_attr($document['id']) ?>" data-link="<?php echo e_attr($document['link']) ?>" data-lien="<?php echo e_attr($document['lien']) ?>" data-nom="<?php echo e_attr($document['nom']) ?>" data-category="<?php echo e_attr($document['category']) ?>">Editer</button>|<a class="btn bnt-xs btn-danger" href="<?php echo base_url('/admin/delete_document/').rawurlencode($document['id']) ?>">Supprimer</button></td>
					</tr>
					<?php endforeach; ?>
				</tbody>
			</table>
